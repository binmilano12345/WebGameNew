...
namespace AssetBundles {
    ...
    public class AssetBundleManager : MonoBehaviour {
        ...
        public static void CheckingDownloadAsset() {
            //			foreach(var dl in m_DownloadingWWWs)
            //			{
            //				Debug.Log ("Downloading " + dl.Key + ": " + (int)(dl.Value.progress * 100) + "%");	
            //			}
        }

        //public static string DownloadingAssetName { get; private set; }

        public static float DownloadingProgress {
            get {
                float totalFinishFile = totalFileDownload - m_DownloadingWWWs.Count;
                float result = totalFinishFile;

                foreach (var dl in m_DownloadingWWWs) {
                    result += dl.Value.progress;
                }

                return result / totalFileDownload;
            }
        }

        private static int totalFileDownload;
        ...
        static protected void LoadAssetBundle(string assetBundleName, bool isLoadingAssetBundleManifest = false) {
            ...
            bool isBundleLoaded = LoadAssetBundleInternal(assetBundleName, isLoadingAssetBundleManifest);
            totalFileDownload = 1;
            // Load dependencies.
			if (!isBundleLoaded && !isLoadingAssetBundleManifest) {
...
//			Debug.LogError("Add download file from Url: " + url);
        ...
        static protected void LoadDependencies(string assetBundleName) {
            ...
            totalFileDownload += dependencies.Length;
            for (int i = 0; i < dependencies.Length; i++)
        ...
        void Update() {
            ...
            foreach (var keyValue in m_DownloadingWWWs) {
                ...
                if (download.isDone) {
					...
					if (keyValue.Key.Equals ("chip"))
						bundleChip = download.assetBundle;
